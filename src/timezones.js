...
(function () {
    ...
    $P.getTimezoneInfo = function () {
        ...
        // If timezone is specified, get the correct timezone info based on the Date given
        if (this.timezone) {
            res = this.timezone === 'Etc/UTC' || this.timezone === 'Etc/GMT' ? { tzOffset: 0, tzAbbr: 'UTC' }: timezoneJS.timezone.getTzInfo(this, this.timezone);
        }
        // If no timezone was specified, use the local browser offset
        else {
            res = { tzOffset: this.getTimezoneOffset(), tzAbbr: null };
        }
        this._tzInfo = res;
        this._useCache = true;
        return res;
